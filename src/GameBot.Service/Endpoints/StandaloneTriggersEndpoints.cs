...
internal static class StandaloneTriggersEndpoints
{
    public static IEndpointRouteBuilder MapStandaloneTriggerEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("/triggers").WithTags("Triggers");

        group.MapGet("/", async (ITriggerRepository repo, CancellationToken ct) =>
        {
            var all = await repo.ListAsync(ct).ConfigureAwait(false);
            var dtos = all.Select(TriggerMappings.ToDto).ToArray();
            return Results.Ok(dtos);
        });

        group.MapGet("/{id}", async (string id, ITriggerRepository repo, CancellationToken ct) =>
        {
            var trigger = await repo.GetAsync(id, ct).ConfigureAwait(false);
            return trigger is null ? Results.NotFound() : Results.Ok(TriggerMappings.ToDto(trigger));
        });

        group.MapPost("/", async (ProfileTriggerCreateDto dto, ITriggerRepository repo, CancellationToken ct) =>
        {
            var trigger = MapCreateDto(dto);
            trigger.EnabledAt = trigger.Enabled ? DateTimeOffset.UtcNow : null;
            await repo.UpsertAsync(trigger, ct).ConfigureAwait(false);
            return Results.Created($"/triggers/{trigger.Id}", TriggerMappings.ToDto(trigger));
        });

        group.MapPatch("/{id}", async (string id, HttpRequest req, ITriggerRepository repo, CancellationToken ct) =>
        {
        ...
        group.MapDelete("/{id}", async (string id, ITriggerRepository repo, CancellationToken ct) =>
        {
            var ok = await repo.DeleteAsync(id, ct).ConfigureAwait(false);
            return ok ? Results.NoContent() : Results.NotFound();
        });

        group.MapPost("/{id}/test", async (string id, ITriggerRepository repo, TriggerEvaluationService svc, CancellationToken ct) =>
        {
            var trig = await repo.GetAsync(id, ct).ConfigureAwait(false);
            if (trig is null) return Results.NotFound();
        ...
        group.MapPost("/evaluate", async (ITriggerRepository repo, TriggerEvaluationService svc, CancellationToken ct) =>
        {
            var all = await repo.ListAsync(ct).ConfigureAwait(false);
            var results = new List<TriggerEvaluationResult>();
            foreach (var trig in all.Where(t => t.Enabled))
            {
...
// (Removed custom DTOs; reusing existing ProfileTriggerCreateDto/ProfileTriggerDto from Models/Triggers.cs)