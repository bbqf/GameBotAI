...
internal static class SessionsEndpoints {
  public static IEndpointRouteBuilder MapSessionEndpoints(this IEndpointRouteBuilder app) {
    app.MapPost("/api/sessions", (CreateSessionRequest req, ISessionManager mgr) => {
      ...
      try {
        ...
        return Results.Created($"/api/sessions/{sess.Id}", resp);
      }
    ...
    app.MapPost("/api/sessions/{id}/execute-action", async (string id, string actionId, IActionRepository actions, ISessionManager mgr, CancellationToken ct) => {
      ...
      if (action.Steps.Count == 0)
        return Results.Accepted($"/api/sessions/{id}", new { accepted = 0 });
