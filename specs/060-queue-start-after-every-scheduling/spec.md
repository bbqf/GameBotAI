...
- Q: How should "after every sequence" relate to the existing "Every Step" option? → A: It is the **same narrow behavior** as today's "Every Step" (fires after each normal "Once Per Run" step only), and it **supersedes** "Every Step" — renamed to **"After Every Step"**. The broad "fires after every executed sequence (start/timer/etc.)" interpretation is **out of scope**. *(Reversed 2026-09-01: the broad interpretation is now the specified behavior — see FR-005a. The narrow one left recovery guards dormant on long-lived non-cycling queues.)*
...
- **FR-005**: ~~"After Every Step" entries MUST continue to execute after each normal ("Once Per Run") step (and once more after the final normal step), in their order of appearance in the template — identical to the current "Every Step" behavior. "After Every Step" MUST NOT fire after "At Queue Start" executions or after timer/relative/live firings.~~ **SUPERSEDED (2026-09-01)** — see FR-005a.
- **FR-005a**: "After Every Step" entries MUST execute after **every** firing a run performs — "At Queue Start", "Once Per Run", time-of-day, relative-offset, live and self-reschedule alike — in their order of appearance in the template. The narrow scoping in the original FR-005 made the option unusable for its main real-world purpose (a recovery/unblock guard): on a long-lived non-cycling queue the once-per-run pass completes minutes after start, after which the entry never fired again for the remaining ~24h of the run, while every timer-driven firing went unguarded. Observed 2026-09-01: an "account logged in on another device" modal blocked emulator-5556 for 4h and 20 consecutive firings while the guard entry sat dormant.
...
- **SC-002**: For a template with an "After Every Step" entry, the entry executes after every firing the run performs (queue-start, normal steps, and timer/relative/live/self-reschedule firings), with zero self-triggered executions, across a cycling and a non-cycling run. *(Amended 2026-09-01 with FR-005a; previously asserted no executions after queue-start or timer firings.)*
