...
- **FR-014a**: System MUST block loading a template into a queue that is in the "running" state (loading is a bulk replacement of all entries); the operator must stop the queue first. The block MUST be indicated through the reachable UI — the Load control is disabled while the queue is running — and enforced by the API, which rejects a load against a running queue with a "stop the queue first" message. Saving a template from a running queue remains allowed.
