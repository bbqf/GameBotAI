...
- [ ] T001 Create `src/GameBot.Service/Services/QueueExecution/QueueCycleLedger.cs` with the records
...
- [ ] T002 Implement the `QueueCycleLedger` class in the same file: one private lock, a private
      `OpenCycle` holder, a `Queue<QueueCycleRecord>` of completed cycles, and the counters.
      `const int MaxRetainedCycles = 50`.
- [ ] T003 Implement the mutators — `EnsureOpen(now)` (idempotent: no-op when a cycle is already
...
- [ ] T004 Implement the readers — `SnapshotHealth()` and `SnapshotCycles(limit)` (newest first, at
...
- [ ] T005 Add a `public QueueCycleLedger Cycles { get; } = new();` to
...
- [ ] T006 In `src/GameBot.Service/Services/QueueExecution/QueueExecutionService.cs`, call
...
- [ ] T007 In the same file, call `handle.Cycles.RecordEntry(<sequenceId>, <ok>)` after **every**
...
- [ ] T008 In the same file, call `handle.Cycles.CompleteOpen(_timeProvider.GetLocalNow())` at the
...
- [ ] T009 In the same file, bracket the roster-pass entry execution with
...
- [ ] T010 In the same file, call `handle.Cycles.RecordEmptyCycle(_timeProvider.GetLocalNow())` in the
      empty-template branch that sets `cycles = 1`, so an idle-but-alive queue still reports one
      completed cycle (spec Edge Cases).
- [ ] T011 Verify by inspection that `RunEveryStepPassAsync` records its firings through T007's call
...
- [ ] T012 [P] [US1] Create `src/GameBot.Service/Contracts/Queues/QueueHealthResponse.cs` with the
...
- [ ] T013 [US1] Add a nullable `QueueHealthResponse? Health` to
      `src/GameBot.Service/Contracts/Queues/QueueDetailResponse.cs`, documented as null when the queue
      is not running.
- [ ] T014 [US1] In `src/GameBot.Service/Endpoints/QueuesEndpoints.cs`, add a static
...
- [ ] T015 [US1] Add a shared static `TryGetLiveRun(id, runtime, registry, out handle)` helper in
...
- [ ] T015a [US1] Populate `Health` in `BuildDetailAsync` via `TryGetLiveRun`, leaving it **null**
      otherwise (FR-008). Add `IQueueRunRegistry` to the `GET {id}` handler's parameters.
- [ ] T016 [P] [US1] Create `tests/unit/QueueCycleLedgerTests.cs` covering: ordinal is 1-based and
...
- [ ] T017 [P] [US1] Create `tests/contract/Queues/QueueCyclesApiContractTests.cs` with the health
...
- [ ] T018 [US1] Add the SC-003 regression test to that file: a run whose cycles all fail reports a
...
- [ ] T019 [P] [US2] Create `src/GameBot.Service/Contracts/Queues/QueueCyclesResponse.cs` containing
      ...
      per data-model.md.
- [ ] T020 [US2] In `QueuesEndpoints.cs`, add a static `ProjectCycles(...)` helper that maps ledger
...
- [ ] T021 [US2] Register `group.MapGet("{id}/cycles", …).WithName("GetQueueCycles")` immediately
...
- [ ] T022 [US2] Implement the handler: 404 for an unknown queue id; `running:false` with an empty
...
- [ ] T023 [US2] Implement `limit` handling — default 20, clamped to 1–50, out-of-range clamped rather
      than rejected (FR-014).
- [ ] T024 [P] [US2] Extend `tests/contract/Queues/QueueCyclesApiContractTests.cs` with the cycles
...
- [ ] T025 [P] [US2] Add a ledger unit test asserting `SnapshotCycles(limit)` returns at most `limit`
...
- [ ] T026 [US3] Add a contract test asserting a non-cycling queue reports exactly one completed cycle
      for its single pass, and that its trailing timer-poll iterations add no further cycles (FR-019).
- [ ] T027 [US3] Add a contract assertion that the terminating execution-log record's status and
      summary text for a queue run are unchanged by this feature (FR-018).
- [ ] T028 [US3] Run the existing `tests/contract/Queues/QueuesApiContractTests.cs`,
...
- [ ] T028a [US3] Add a contract test for the FR-008a gate: a queue reporting `status: "Stopped"`
      never carries a non-null `health`, and `GET {id}` and `GET {id}/cycles` agree about whether the
      run is in progress.
- [ ] T028b [US3] Add a contract test for the concurrent-queues edge case: two queues running on
...
- [ ] T029 [P] Update `src/GameBot.Service/Swagger/SwaggerConfig.cs` to document the `{id}/cycles`
      route (including the `limit` parameter and its clamping) and the additive `health` block.
- [ ] T030 Update `docs/architecture.md`: add the `health` block and the `{id}/cycles` route to the
      queue API surface, note that cycle history is run-scoped and does not survive a restart, and
      **refresh the "Last reviewed" date**.
- [ ] T031 [P] Set `**Status**: Implemented` in `specs/086-queue-cycle-observability/spec.md` and add
      the 086 row to `specs/STATUS.md`. Feature 072 (live monitor) is complemented, not superseded —
      leave its status alone.
- [ ] T032 Build the solution and run both test projects; confirm zero warnings introduced and all
      tests green before committing (constitution: "an evaluation of test builds and runs results is
      necessary before each commit").
- [ ] T033 Verify the quickstart flow in `quickstart.md` reads correctly against the implemented
