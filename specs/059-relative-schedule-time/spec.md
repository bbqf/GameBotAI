...
- **Very large offset that never elapses during the run**: A run with a still-pending relative-offset timer (or live schedule) stays active rather than completing immediately — including a non-cyclic run, which would otherwise finish in a single fast pass. The run keeps waiting (polling at a short interval) until the offset elapses and the sequence fires, or until the operator stops the run. The sequence only never fires if the run is stopped before the offset elapses. (Absolute time-of-day timers retain their feature-053 behavior and do not, on their own, keep a non-cyclic run alive.)
