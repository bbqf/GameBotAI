...
- Q: What should the default install roots be by scope? → A: Per-machine uses `%ProgramFiles%\GameBot`; per-user uses `%LocalAppData%\GameBot`.
...
- Installer supports both installation scopes in v1: per-machine and per-user
- Installer enforces scope constraints by mode: service mode requires per-machine scope; background-app mode supports per-user and per-machine scope
...
- Installer supports background-app mode with non-admin runtime execution for per-user installs; per-machine background installs require elevation during installation but runtime execution does not require admin rights
...
- Installer uses scope-specific default install roots: per-machine `%ProgramFiles%\GameBot`, per-user `%LocalAppData%\GameBot`
- Installer uses scope-specific runtime data roots outside `%ProgramFiles%`: per-machine `%ProgramData%\GameBot\data`, per-user `%LocalAppData%\GameBot\data`
- Installer exposes runtime data directory in canonical silent property schema as `DATA_ROOT`, defaulted by selected install scope
...
- Installer registers runtime startup behavior by selected mode: service mode registers auto-start service; background-app mode registers per-user startup entry
...
- Installer validates selected scope/mode combinations and blocks invalid combinations with actionable error messaging
...
- Default install location is applied correctly by selected scope unless operator overrides it
- Runtime data directory is created at scope-appropriate path and is writable by the selected install mode context
