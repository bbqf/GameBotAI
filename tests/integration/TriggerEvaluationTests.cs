...
internal class TriggerEvaluationTests {
  ...
  public async Task ImageTriggerTestEndpointReturnsPendingWithoutScreen() {
    ...
    // Create game
    var gameResp = await client.PostAsJsonAsync(new Uri("/games", UriKind.Relative), new { name = "G", description = "d" }).ConfigureAwait(false);
  ...
  public async Task DelayTriggerRespectsCooldown() {
    ...
    // Create game
    var gameResp = await client.PostAsJsonAsync(new Uri("/games", UriKind.Relative), new { name = "G2", description = "d" }).ConfigureAwait(false);
    gameResp.StatusCode.Should().Be(HttpStatusCode.Created);
    var game = await gameResp.Content.ReadFromJsonAsync<Dictionary<string, object>>().ConfigureAwait(false);
    var gameId = game!["id"]!.ToString();
    ...
    var testResp1 = await client.PostAsync(new Uri($"/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
    ...
    var testResp2 = await client.PostAsync(new Uri($"/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
    ...
    var testResp3 = await client.PostAsync(new Uri($"/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
