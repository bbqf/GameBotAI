...
public class TriggerEvaluationTests {
  ...
  public async Task ImageTriggerTestEndpointReturnsPendingWithoutScreen() {
    ...
    // Create game
    var gameResp = await client.PostAsJsonAsync(new Uri("/api/games", UriKind.Relative), new { name = "G", description = "d" }).ConfigureAwait(false);
  ...
  public async Task DelayTriggerRespectsCooldown() {
    ...
    // Create game
    var gameResp = await client.PostAsJsonAsync(new Uri("/api/games", UriKind.Relative), new { name = "G2", description = "d" }).ConfigureAwait(false);
    gameResp.StatusCode.Should().Be(HttpStatusCode.Created);
    var game = await gameResp.Content.ReadFromJsonAsync<Dictionary<string, object>>().ConfigureAwait(false);
    var gameId = game!["id"]!.ToString();
    ...
    var testResp1 = await client.PostAsync(new Uri($"/api/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
    testResp1.StatusCode.Should().Be(HttpStatusCode.OK);
    var res1 = await testResp1.Content.ReadFromJsonAsync<Dictionary<string, object>>().ConfigureAwait(false);
    ((System.Text.Json.JsonElement)res1!["status"]).GetString().Should().Be("Satisfied");

    // Immediate second test -> Cooldown
    var testResp2 = await client.PostAsync(new Uri($"/api/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
    testResp2.StatusCode.Should().Be(HttpStatusCode.OK);
    var res2 = await testResp2.Content.ReadFromJsonAsync<Dictionary<string, object>>().ConfigureAwait(false);
    ...
    var testResp3 = await client.PostAsync(new Uri($"/api/triggers/{triggerId}/test", UriKind.Relative), null).ConfigureAwait(false);
    testResp3.StatusCode.Should().Be(HttpStatusCode.OK);
    var res3 = await testResp3.Content.ReadFromJsonAsync<Dictionary<string, object>>().ConfigureAwait(false);
