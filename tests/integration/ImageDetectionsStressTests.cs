...
public sealed class ImageDetectionsStressTests {
  ...
  public async Task ManyMatchesAreCappedAndLimitsHitFlagSet() {
    ...
    var up = await client.PostAsJsonAsync(new Uri("/images", UriKind.Relative), new { id = "tplStress", data = TwoByTwoPatternB64() });
    up.StatusCode.Should().Be(HttpStatusCode.Created);

    var resp = await client.PostAsJsonAsync(new Uri("/images/detect", UriKind.Relative), new { referenceImageId = "tplStress", threshold = 0.5, maxResults = 1 });
  ...
  public async Task TimeoutReturnsOkWithLimitsHit() {
    ...
    try {
      var resp = await client.PostAsJsonAsync(new Uri("/images/detect", UriKind.Relative), new { referenceImageId = "missing" });
      ...
      var up = await client.PostAsJsonAsync(new Uri("/images", UriKind.Relative), new { id = "tplTimeout", data = oneByOne });
      up.StatusCode.Should().Be(HttpStatusCode.Created);
      ...
      var resp2 = await client.PostAsJsonAsync(new Uri("/images/detect", UriKind.Relative), new { referenceImageId = "tplTimeout" });
