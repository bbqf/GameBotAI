...
public sealed class ListImagesTests : IDisposable
{
  ...
  public ListImagesTests()
  {
    ...
    _dataDir = Path.Combine(Path.GetTempPath(), $"gamebot-contract-images-{Guid.NewGuid():N}");
  ...
  public async Task GetImagesReturnsIdsOnly()
  {
    ...
    var resp = await client.GetAsync(new Uri("/api/images", UriKind.Relative));
    resp.StatusCode.Should().Be(HttpStatusCode.OK);

    var payload = await resp.Content.ReadFromJsonAsync<JsonElement>();
    payload.TryGetProperty("ids", out var idsElement).Should().BeTrue();
    idsElement.ValueKind.Should().Be(JsonValueKind.Array);
    ...
    ids.Should().BeEquivalentTo(SeedIds);
  }
}
